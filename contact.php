<?php
require("./partials/header-1.php");
?>
<!-- header close -->

<!-- subheader begin -->
<section id="subheader" class="page-contact no-bottom" data-stellar-background-ratio="0.5">
    <div class="inner">
        <div class="container">
            <div class="row">
                <div class="col-md-12">
                    <h1>Contact</h1>
                    <div class="crumb">
                        <ul>
                            <li><a href="index.php">Home</a></li>
                            <li>Contact</li>
                        </ul>
                    </div>
                </div>
            </div>
        </div>
    </div>
</section>
<!-- subheader close -->



<!-- content begin -->
<div id="content" class="no-top no-bottom">
    <section>

        <div class="container">
            <div class="row">
                <div class="col-md-12">

                    <div class="expand-box location">
                        <div class="inner">

                            <div class="row">
                                <div class="col-md-2">
                                    Head Quarter
                                    <h3>New York,<br>
                                        USA</h3>
                                </div>
                                <div class="col-md-2">
                                    <h5>Address</h5>
                                    777 Street Fourth Avenue Somewhere Over Rainbow<br>
                                    <span class="btn-arrow btn-open-map"><span class="line"></span><span class="url">View on Map</span></span>

                                </div>
                                <div class="col-md-2">
                                    <h5>Phone</h5>
                                    +22 090 1777 1999<br>
                                    +22 090 1777 1999
                                </div>
                                <div class="col-md-2">
                                    <h5>Fax</h5>
                                    (77) 2222 0900 1777<br>
                                    (77) 2222 0900 1777
                                </div>
                                <div class="col-md-2">
                                    <h5>Email</h5>
                                    office@exotheme.com
                                </div>
                                <div class="col-md-2">
                                    <h5>Social Media</h5>
                                    <!-- social icons -->
                                    <div class="social">
                                        <a href="#"><i class="fa fa-facebook"></i></a>
                                        <a href="#"><i class="fa fa-twitter"></i></a>
                                        <a href="#"><i class="fa fa-rss"></i></a>
                                        <a href="#"><i class="fa fa-google-plus"></i></a>
                                        <a href="#"><i class="fa fa-envelope-o"></i></a>
                                    </div>
                                    <!-- social icons close -->
                                </div>

                            </div>
                        </div>

                        <div class="map-container">
                            <div id="map-1"></div>
                        </div>
                    </div>

                    <div class="expand-box location">
                        <div class="inner">

                            <div class="row">
                                <div class="col-md-2">
                                    Brand Office
                                    <h3>Munich,<br>
                                        Germany</h3>
                                </div>
                                <div class="col-md-2">
                                    <h5>Address</h5>
                                    777 Street Fourth Avenue Somewhere Over Rainbow<br>
                                    <span class="btn-arrow btn-open-map"><span class="line"></span><span class="url">View on Map</span></span>

                                </div>
                                <div class="col-md-2">
                                    <h5>Phone</h5>
                                    +22 090 1777 1999<br>
                                    +22 090 1777 1999
                                </div>
                                <div class="col-md-2">
                                    <h5>Fax</h5>
                                    (77) 2222 0900 1777<br>
                                    (77) 2222 0900 1777
                                </div>
                                <div class="col-md-2">
                                    <h5>Email</h5>
                                    office@exotheme.com
                                </div>
                                <div class="col-md-2">
                                    <h5>Social Media</h5>
                                    <!-- social icons -->
                                    <div class="social">
                                        <a href="#"><i class="fa fa-facebook"></i></a>
                                        <a href="#"><i class="fa fa-twitter"></i></a>
                                        <a href="#"><i class="fa fa-rss"></i></a>
                                        <a href="#"><i class="fa fa-google-plus"></i></a>
                                        <a href="#"><i class="fa fa-envelope-o"></i></a>
                                    </div>
                                    <!-- social icons close -->
                                </div>

                            </div>
                        </div>

                        <div class="map-container">
                            <div id="map-2"></div>
                        </div>
                    </div>

                    <div class="expand-box location">
                        <div class="inner">

                            <div class="row">
                                <div class="col-md-2">
                                    Head Quarter
                                    <h3>London,<br>
                                        England</h3>
                                </div>
                                <div class="col-md-2">
                                    <h5>Address</h5>
                                    777 Street Fourth Avenue Somewhere Over Rainbow<br>
                                    <span class="btn-arrow btn-open-map"><span class="line"></span><span class="url">View on Map</span></span>

                                </div>
                                <div class="col-md-2">
                                    <h5>Phone</h5>
                                    +22 090 1777 1999<br>
                                    +22 090 1777 1999
                                </div>
                                <div class="col-md-2">
                                    <h5>Fax</h5>
                                    (77) 2222 0900 1777<br>
                                    (77) 2222 0900 1777
                                </div>
                                <div class="col-md-2">
                                    <h5>Email</h5>
                                    office@exotheme.com
                                </div>
                                <div class="col-md-2">
                                    <h5>Social Media</h5>
                                    <!-- social icons -->
                                    <div class="social">
                                        <a href="#"><i class="fa fa-facebook"></i></a>
                                        <a href="#"><i class="fa fa-twitter"></i></a>
                                        <a href="#"><i class="fa fa-rss"></i></a>
                                        <a href="#"><i class="fa fa-google-plus"></i></a>
                                        <a href="#"><i class="fa fa-envelope-o"></i></a>
                                    </div>
                                    <!-- social icons close -->
                                </div>

                            </div>
                        </div>

                        <div class="map-container">
                            <div id="map-3"></div>
                        </div>
                    </div>

                </div>
            </div>
        </div>

    </section>

    <section id="section-contact-form">
        <div class="container">
            <div class="row">
                <div class="col-md-4">
                    <div class="light-text">
                        <div class="bg-2 padding30">

                            <h2 class="id-color">Gocargo Commitment</h2>
                            <div class="tiny-border"></div>
                            <p class="lead big">
                                <i>Fell free to asking about Gocargo or Just say hello to us </i>
                            </p>
                            <div class="text-center">
                                <img src="img/contact/truck.png" alt="">
                            </div>
                        </div>
                    </div>

                </div>

                <div class="col-md-8">
                    <form id="contact" class="row form-transparent" name="form1" method="post" action="mail.php">

                        <div class="col-md-6">
                            <input  required type="text" class="form-control" name="fname" id="name" placeholder="First Name*" />
                        </div>

                        <div class="col-md-6">
                            <input  required type="text" class="form-control" name="lname" id="last_name" placeholder="Last Name*" />
                        </div>

                        <div class="col-md-6">
                            <div id="error_email" class="error">Please check your email</div>
                            <input required  type="email" class="form-control" name="email" id="email" placeholder="Email*" />
                        </div>

                        <div class="col-md-6">
                            <div id="error_email" class="error">Please check your email</div>
                            <input required  type="text" class="form-control" name="phone" id="phone" placeholder="Phone*" />
                        </div>

                        <div class="col-md-12">
                            <div id="error_message" class="error">Please check your message</div>
                            <textarea required  cols="10" rows="10" name="message" id="message" class="form-control" placeholder="Your message"></textarea>
                        </div>

                        <div id="mail_success" class="col-md-12 success">Thank you. Your message has been sent.</div>
                        <div id="mail_failed" class="col-md-12 error">Error, email not sent</div>

                        <div class="col-md-12">
                            <p id="btnsubmit">
                                <input required  type="submit" id="send" value="Send" class="btn btn-custom fullwidth" />
                            </p>
                        </div>

                    </form>
                </div>
            </div>
        </div>
    </section>

</div>
</div>
<!-- content close -->
<?php
require("./partials/footer.php");
?>